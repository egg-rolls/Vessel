...
/** 构造事件（ADR-030：事件名为开放字符串字面量，无常量/枚举） */
function makeEvent(type: string, runId: string, overrides: Partial<RunEvent> = {}): RunEvent {
  return {
    type,
    run_id: runId,
...
/** 过滤掉回放标记事件，只保留原始事件 */
function stripReplayMarkers(events: RunEvent[]): RunEvent[] {
  return events.filter((e) => e.type !== 'replay.started' && e.type !== 'replay.completed');
}
...
describe('FileEventStore', () => {
  ...
  it('should append and read back events', async () => {
    ...
    expect(read[0]?.type).toBe('run.started');
  ...
  it('should list run IDs from persisted events', async () => {
    await store.append(makeEvent('run.started', 'run-a'));
    await store.append(makeEvent('run.completed', 'run-a'));
    await store.append(makeEvent('run.started', 'run-b'));
  ...
  it('should skip malformed JSON lines', async () => {
    // 直接写一行坏数据 + 一行好数据
    const event = makeEvent('run.completed', 'run-1');
    await store.append(event);
    // 手动插入一行坏数据
...
describe('replayRun', () => {
  ...
  it('should replay events to the event stream', async () => {
    await store.append(makeEvent('run.started', 'run-1', { ts: 1000 }));
    await store.append(makeEvent('run.completed', 'run-1', { ts: 2000 }));
    ...
    // 过滤掉标记事件，只看原始事件
    const replayed = stripReplayMarkers(received);
    expect(replayed).toHaveLength(2);
    expect(replayed[0]?.type).toBe('run.started');
    expect(replayed[1]?.type).toBe('run.completed');
  });

  it('should emit replay.started before and replay.completed after replayed events', async () => {
    await store.append(makeEvent('run.started', 'run-1', { ts: 1000 }));
    ...
    expect(received[0]?.type).toBe('replay.started');
    expect(received[received.length - 1]?.type).toBe('replay.completed');
  });

  it('should preserve original timestamps on replayed events', async () => {
    const originalTs = 1699999999999;
    await store.append(makeEvent('run.started', 'run-1', { ts: originalTs }));

    const received = collectEvents(eventStream);

    await replayRun(store, 'run-1', eventStream, { delayMs: 0 });

    const replayed = received.find((e) => e.type === 'run.started');
  ...
  it('should replay events for the correct run only', async () => {
    await store.append(makeEvent('run.started', 'run-1', { ts: 1000 }));
    await store.append(makeEvent('run.started', 'run-2', { ts: 2000 }));
    await store.append(makeEvent('run.completed', 'run-1', { ts: 3000 }));
...
describe('Event replay integration', () => {
  ...
  it('should produce identical event sequence on replay', async () => {
    ...
    // 5. 验证回放事件序列（排除 replay.started/replay.completed）
    const replayed = stripReplayMarkers(subscribed);
