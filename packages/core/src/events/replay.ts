 ...
 *
 * 先发布 `replay.started` 信号，再按序发布每个原始事件（保留原始 ts），
 * 最后发布 `replay.completed` 信号。订阅者通过同一 EventStream.subscribe()
 * 接收，不区分 live/replay。
 *
 * 事件名为开放字符串字面量（ADR-030）——与 `run.started` 等同级，零枚举、零 core 事件类型变更。
 *
 * @param store - 事件持久化存储
 * @param runId - 要回放的 run_id
 * @param eventStream - 目标 EventStream
 * @param options - 可选配置（delayMs 默认 50ms）
 * @returns 回放的事件数量（不含 replay.started/replay.completed 标记），未找到返回 0
 */
