  ...
  if (config.plugins) {
    config.plugins.forEach((plugin, index) => {
      ...
      if (!Array.isArray(servers)) {
        errors.push({
          path: `plugins[${index}].config.servers`,
      ...
      servers.forEach((server, serverIndex) => {
        if (!server || typeof server !== 'object') {
          errors.push({
            path: `plugins[${index}].config.servers[${serverIndex}]`,
        ...
        if (typeof entry.name !== 'string' || entry.name.trim() === '')
          errors.push({
            path: `plugins[${index}].config.servers[${serverIndex}].name`,
            message: 'MCP server name must be a non-empty string',
            value: entry.name,
          });
        if (typeof entry.command !== 'string' || entry.command.trim() === '')
          errors.push({
            path: `plugins[${index}].config.servers[${serverIndex}].command`,
            message: 'MCP server command must be a non-empty string',
            value: entry.command,
          });
      });
    });
