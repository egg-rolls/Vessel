...
export const McpBrowser: React.FC<{ ctx: ReplContext; onClose: () => void }> = ({
  ctx,
  onClose,
}) => {
  ...
  const refresh = useCallback(
    () => void new DashboardService(ctx).getAssets().then((data) => setItems(data.mcpServers)),
    [ctx],
  );
  useEffect(() => {
    refresh();
  ...
  useInput((input, key) => {
    ...
    if (!item) return;
    if (input.toLowerCase() === 'r' || input.toLowerCase() === 'c')
      void ctx.mcpController?.reconnect(item.name).then(refresh);
  ...
  const item = items[selected];
  const serverTools = item
    ? ctx.tools.list().filter((tool) => tool.name.startsWith(`mcp__${item.name}__`))
    : [];
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="blue" paddingX={1} flexGrow={1}>
      <Text bold color="blue">
        {details && item ? `MCP: ${item.name}` : 'MCP Servers'}
      </Text>
      {!details &&
        items.map((server, index) => (
          <Text key={server.name} color={index === selected ? 'cyan' : undefined}>
            {index === selected ? '❯ ' : '  '}
      ...
      {details && item && (
        <Box flexDirection="column">
          ...
          {testResult && (
            <Text color={testResult.startsWith('Error') ? 'red' : 'yellow'} wrap="truncate">
              Test │ {testResult}
            </Text>
          )}
          <Text color="blue">Tools</Text>
          {serverTools.map((tool) => (
            <Text key={tool.name} color="gray" wrap="truncate">
              {' '}
              - {tool.name.split(`mcp__${item.name}__`)[1]} {tool.description}
            </Text>
          ))}
        </Box>
      )}
      <Text color="gray">
        ↑↓ Navigate · Enter Details · R Reconnect · T Test · D Disconnect · Esc Back
      </Text>
    </Box>
  );
};
