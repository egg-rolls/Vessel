...
全局约束（[AGENTS.md](AGENTS.md) 的红线、能力分层、Core 冻结）由子角色 TODO 第 1 步统一加载——本层不重复列出，只负责把请求分发到正确角色。

### TodoList 模板

```
1. 判断角色：用户的请求属于哪种场景？（对照下方场景调度表）
2. 读对应的子角色 TODO 文件 — 加载第二套 TodoList，将用户原始需求插入到子 TODO 的 `[用户需求]` 占位符处
3. 检查分支：`git branch --show-current`
4. 按子角色 TODO 逐步执行 — 过程中按需查阅对应的角色详细说明文档
```
