...
> **⚡ 操作前硬自检（每次改代码/文档前，逐条过，不跳过）**：
> 
> ```
> [ ] 分支对吗？—— git branch --show-current，当前分支名是否匹配任务？
>     不是 → 切到正确分支或从 main 新建。改完才发现在错误分支 = 浪费时间。
> [ ] 场景文档读了吗？—— 对照上方「场景调度」表，找到当前任务类型对应的必读项，读了。
>     没读 → 先读再动手。跳过前置阅读 = 写出来的大概率违规。
> [ ] 任务范围明确吗？—— 能用一句话说出"我要改哪个文件、改什么、为什么"。
>     说不清 → 先和用户确认，不问就做 = 做出来不对。
> ```
> 
> 这三问不是设计判断——是 10 秒的机械检查。花 10 秒省 20 分钟返工。
...
审查分两层：**硬性门禁**是 binary check（过/不过）；**设计判断**需要读代码、动用判断力。
...
#### 设计判断（读 [§4.1](#41-代码设计纪律前期投入换长期回报) + [`GIT-WORKFLOW.md` §六](docs/specs/GIT-WORKFLOW.md) 后做出判断）

这部分不是 checkbox——是审查时问自己的问题：

- 新增代码的边界清晰吗？每个核心逻辑能**独立测试**吗？
- 加了新 case / 事件 / 工具——是只加不改，还是趟了一遍核心分发？
- 有没有**抄近路**的信号：超长函数、重复逻辑、散落魔法值、`as` 满屏飞？
- 这些信号是否有**有意识的设计决策**支撑？（"这里不提取是因为……"）
