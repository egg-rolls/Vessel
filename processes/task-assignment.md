...
```
切分支 → AI 辅助开发 → 自查门禁 → 推 PR → 对方 Review → 合并
```

### Review 规则

- 李瑞的 PR → 贾斯涵 Review（检查实现是否与设计意图一致）
- 贾斯涵的 PR → 李瑞 Review（检查架构合规、红线、门禁）
- 每人每天最多 2 个 PR 待审查，避免堆积
...
| ID | 任务 | 类型 | 说明 |
|----|------|------|------|
| A1 | 统一工具注册路径 | 架构重构 | 决定 ToolRegistry 与 PluginHost 如何合并；产出设计方案后贾斯涵实现 |
...
| A5 | 合并门禁执行 | 流程 | 审查贾斯涵的所有 PR，确保过门禁；维护 CLAUDE.md 与 specs 一致性 |
...
第一轮（本周，独立并行）：
  ...
  李瑞 A1              贾斯涵 B1 + B2 + B3
  （出设计文档）        （3 个小修复，可独立提 PR）


第二轮：

  李瑞 A2 + A4          贾斯涵 B4 + B5 + B6
  （出设计文档）        （3 个中等修复）


第三轮：

  李瑞 Review A1-A4    贾斯涵 B7
...
2. **沟通**用 PR comment + Commit message，不额外开会
