...
- [x] P3-1：**StreamRenderer 单元测试**——`stream-renderer.ts` 的 `handleEvent` switch、`handleChunk` 解析、`didStreamLastRun` 状态、颜色开关、工具卡片渲染。22 个测试已通过。
- [x] P3-2：**`/help` 动态化**——`commands.ts` 已改为读取 `CommandRegistry.list()` 和子命令动态生成帮助文本，加命令即生效。
- [x] P3-3：**确认期间缓冲输入**——`repl.ts` 的 `pausedForConfirm` 标志已改为缓冲输入，确认结束后重新输入。
- [x] P3-4：**`/setup` 热替换 provider**（带 `/reload` 命令）——已添加 `/reload` 命令，可在进程内重新加载配置。

### emma Ink REPL 框架（额外完成）

emma 已完成 Ink REPL 框架实现，包括：
...
- [x] 所有命令 output 字段支持（Ink REPL 可显示命令输出）

### 第三梯队：emma 接口文档

- [x] DOC-1：`docs/api/tui.md`——接口文档 + 替换指南 + 注意事项（egg-rolls 已交付）
- [x] DOC-2：`docs/guides/` 更新（已更新测试数量和测试结构）
